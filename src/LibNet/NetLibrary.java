...
public static PetriNet CreateNetSMOgroup(int numInGroup,int numChannel, double timeMean, String name) throws ExceptionInvalidNetStructure, ExceptionInvalidTimeDelay{
	...
	public static PetriNet CreateNetmodel(int operators, int drivers) throws ExceptionInvalidNetStructure, ExceptionInvalidTimeDelay {
		double[] distances = { 5, 8, 9, 11, 12, 20 };
		double[] distanceProbabilities = { 0.1, 0.2, 0.25, 0.17, 0.23, 0.05 };
		...
		d_T.add(new PetriT("Надходження викликів",3.0));
		d_T.get(0).setDistribution("erl", d_T.get(0).getTimeServ());
		d_T.get(0).setParamDeviation(2.0);
		d_T.add(new PetriT("Набирання номера",0.5));
		d_T.add(new PetriT("Спроба замовлення таксі",1.0));
		...
		d_T.add(new PetriT("Очікування повторного набирання",1.0));
		d_T.add(new PetriT("Набирання номера",0.5));
		d_T.add(new PetriT("Спроба замовлення таксі",1.0));
		...
		d_T.add(new PetriT("Обслуговування клієнта",40.0));
